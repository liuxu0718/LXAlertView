...
- (id)initWithTitle:(NSString *)title Type:(LXAlertViewType)type {
  if (self = [super init]) {
    ...
    if (LXAlertViewTypeDefault == _type || LXAlertViewTypeLand == _type) {
      ...
      _confirmButton.frame = CGRectMake(0,
                                        _backgroundView.frame.size.height - 45,
      ...
      _titleLabel.frame = CGRectMake(0,
                                     0,
                                     _backgroundView.frame.size.width,
                                     (_backgroundView.frame.size.height - 45) / 2);
      //设置成文字
      [_closeButton setTitle:@"取消" forState:UIControlStateNormal];
      [_confirmButton setTitle:@"确认" forState:UIControlStateNormal];
    }
...
-(UITextField *)textField {
  if (!_textField) {
    ...
    _textField.frame                = CGRectMake(0,
                                                 (_backgroundView.frame.size.height - 45) / 2,
                                                 _backgroundView.frame.size.width - 50,
                                                 30);
...
-(UIButton *)confirmButton {
  if (!_confirmButton) {
    ...
    _confirmButton.frame            = CGRectMake(_backgroundView.frame.size.width / 2,
                                                 _backgroundView.frame.size.height - 45,
                                                 _backgroundView.frame.size.width / 2,
                                                 45);
